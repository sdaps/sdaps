...
def create_stamp_pdf(survey, questionnaire_ids):
	...
	try:
		...
		subprocess.call(['pdflatex', '-draftmode', '-halt-on-error', '-interaction', 'batchmode', '-output-directory', tmpdir, os.path.join(tmpdir, 'questionnaire.tex')])
		# And again, without the draft mode
		subprocess.call(['pdflatex', '-halt-on-error', '-interaction', 'batchmode', '-output-directory', tmpdir, os.path.join(tmpdir, 'questionnaire.tex')])
