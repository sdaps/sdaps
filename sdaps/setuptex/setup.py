...
def setup (survey, questionnaire_tex, additionalqobjects = None) :
	...
	try:
		...
		subprocess.call(['pdflatex', '-draftmode', '-halt-on-error', '-interaction', 'batchmode', 'questionnaire.tex'], cwd=survey.path())
		# And again, without the draft mode
		subprocess.call(['pdflatex', '-halt-on-error', '-interaction', 'batchmode', 'questionnaire.tex'], cwd=survey.path())
